...
IC_puntos = zeros(mtp,12); %matriz de IC: para cada mes para cada punto
for i =  1:mtp
    for j = 1:12
            IC_puntos(i,j) = IC_puntos(i,j) + (temps_p(i,j+4)/5)^1.514;
    end
end
...
for i = 1:mtp-1
    for j = 1:12
        ETo(i,j) = 16*d_puntos(i,j)*(10*abs(temps_p(i,j+4))/IC_puntos(i,j))^a_puntos(i,j); %Evapotranspiración para cada punto para cada mes
        ETo_total(j,1) = ETo_total(j,1) + ETo(i,j); %Suma de toda la evapotranspiración 
    end
end

%xlswrite("evapotranspiracion_punto_mes_prueba_2.xlsx",ETo);
